...
export const ReportHazardView: React.FC<ReportHazardViewProps> = ({
  ...
  const handleSubmitReport = async () => {
    ...
    let plate = vehiclePlateNumber.trim();

    if (isTraffic && !plate) {
      const useCctv = confirm(
        '⚠️ Vehicle License Plate Required:\n\nNo plate was detected from this photo. If the vehicle was distant, moving, or blurred, would you like to request Traffic Police CCTV Verification (officers will trace the vehicle using street/junction cameras)?\n\nClick OK to request CCTV verification and submit.'
      );
      if (useCctv) {
        plate = 'CCTV-VERIFY';
        setVehiclePlateNumber('CCTV-VERIFY');
      } else {
        return;
      }
    }
            ...
            return (
              <div className={`p-4 rounded-2xl space-y-3 transition-all ${
                ...
                <div className="flex items-center justify-between flex-wrap gap-2">
                  ...
                  {isMissingPlate && (
                    <span className="px-2.5 py-0.5 bg-amber-600 text-white font-extrabold text-[10px] rounded-full uppercase tracking-wider">
                      Plate Input or CCTV Request Needed
                    </span>
                  )}
                </div>

                {isMissingPlate ? (
                  <div className="p-3 bg-amber-50/90 border border-amber-300 text-amber-950 font-bold text-xs rounded-xl space-y-2.5">
                    <div className="flex items-start space-x-2">
                      <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                      <div>
                        <span><strong>Vehicle Plate Input / Camera Trace:</strong> If the vehicle plate is visible, enter it below. If the car was distant, in motion, or blurred (e.g. CCTV still or dashcam), click below to request Traffic Police junction camera trace:</span>
                      </div>
                    </div>
                    <div className="pt-0.5">
                      <button
                        type="button"
                        onClick={() => setVehiclePlateNumber('CCTV-VERIFY')}
                        className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-black flex items-center space-x-1.5 shadow-sm transition-all"
                      >
                        <Camera className="w-3.5 h-3.5" />
                        <span>📷 Plate Distant or Blurred? Request Traffic Police CCTV Verification</span>
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    ...
                    {vehiclePlateNumber.toUpperCase() === 'CCTV-VERIFY' && (
                      <span className="text-[11px] font-extrabold text-indigo-700 bg-indigo-100 px-2.5 py-0.5 rounded-full border border-indigo-200 flex items-center space-x-1">
                        <Camera className="w-3 h-3 text-indigo-600 mr-1" />
                        Traffic Police CCTV Camera Investigation Requested
                      </span>
                    )}
                  </div>
                )}
                ...
                <div className="flex flex-col items-center sm:items-end w-full sm:w-auto space-y-1">
                  ...
                  {isInvalidHazard ? (
                    ...
                    <span className="text-[11px] font-extrabold text-amber-700">
                      ⚠️ Enter vehicle number plate or click "Request CCTV Verification" above
                    </span>
                  ) : null}
                </div>
              </div>
